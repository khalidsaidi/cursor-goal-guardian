...
function getGuardianPaths(workspaceRoot2) {
  const dir = import_node_path.default.join(workspaceRoot2, ".cursor", "goal-guardian");
  const telemetryDir = import_node_path.default.join(dir, "telemetry");
  return {
    workspaceRoot: workspaceRoot2,
    dir,
    contract: import_node_path.default.join(dir, "contract.json"),
    config: import_node_path.default.join(dir, "config.json"),
    state: import_node_path.default.join(dir, "state.json"),
    actions: import_node_path.default.join(dir, "actions.jsonl"),
    snapshot: import_node_path.default.join(dir, "snapshot.json"),
    telemetryDir,
    audit: import_node_path.default.join(telemetryDir, "audit.jsonl"),
    verdicts: import_node_path.default.join(telemetryDir, "verdicts.json"),
    episodes: import_node_path.default.join(telemetryDir, "episodes.json"),
...
function newId(prefix) {
  return `${prefix}_${import_node_crypto.default.randomBytes(8).toString("hex")}`;
}
...
function rulesFor(kind, config) {
  const user = kind === "shell" ? config.advisories.shellRules : kind === "mcp" ? config.advisories.mcpRules : config.advisories.readRules;
...
function evaluateLexicalDrift(state, config, actionType, actionValue) {
  if (!config.drift.lexical.enabled)
    return null;
  if (!state)
    return null;
...
function emptyEpisodeStore() {
  return { schemaVersion: 2, episodes: [] };
}
var EPISODE_TTL_MS = 24 * 60 * 60 * 1e3;
var MAX_EPISODE_TERMS = 24;
function overlaps(a, b) {
  const set = new Set(a);
  return b.some((t) => set.has(t));
}
function assignEpisode(store, drift, config, clock = systemClock) {
  const now = clock.now().getTime();
  const ts = nowIso(clock);
  const cooldownMs = config.nudgeCooldownMinutes * 60 * 1e3;
  const episodes = store.episodes.filter((e) => now - Date.parse(e.lastSeenTs) <= EPISODE_TTL_MS);
  const match2 = episodes.find((e) => e.taskId === drift.taskId && now - Date.parse(e.lastSeenTs) <= cooldownMs && (drift.terms.length === 0 || e.terms.length === 0 || overlaps(e.terms, drift.terms)));
  const decideNudge = (lastNudgeTs) => {
    if (config.notify === "quiet")
      return false;
    if (config.notify === "vocal")
      return true;
    if (lastNudgeTs === null)
      return true;
  ...
  if (match2) {
    const shouldNudge2 = decideNudge(match2.lastNudgeTs);
    const updated = {
      ...match2,
      terms: [.../* @__PURE__ */ new Set([...match2.terms, ...drift.terms])].slice(0, MAX_EPISODE_TERMS),
      lastSeenTs: ts,
      lastNudgeTs: shouldNudge2 ? ts : match2.lastNudgeTs
    };
    return {
      episodeId: match2.id,
      shouldNudge: shouldNudge2,
      store: { schemaVersion: 2, episodes: episodes.map((e) => e.id === match2.id ? updated : e) }
    };
  }
...
async function loadEpisodes(workspaceRoot2) {
  const p = getGuardianPaths(workspaceRoot2);
  try {
...
async function saveEpisodes(workspaceRoot2, store) {
  const p = getGuardianPaths(workspaceRoot2);
...
// packages/hook/src/pipeline.ts
function hasActiveDoingTask(state) {
